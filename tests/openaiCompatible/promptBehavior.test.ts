...
test('agent prompt stays compact and workspace-first', () => {
  ...
  assert.match(prompt, /Follow this priority order: system instructions, developer instructions, user request, then repository instructions included in the prompt\./u)
  assert.match(prompt, /repository instructions already included in context as available context rather than something to rediscover/u)
  ...
  assert.match(prompt, /If a tool result already gives a complete answer, reuse it instead of rereading the same file or range unless the file changed or the gap is still unresolved\./u)
  assert.match(prompt, /When creating or editing source files, keep the file formatted with normal multiline structure and indentation instead of collapsing code into a single line\./u)
...
test('plan prompt stays compact and scope-first', () => {
  ...
  assert.match(prompt, /If a tool result already gives a complete answer, reuse it instead of rereading the same file or range unless the file changed or the gap is still unresolved\./u)
  assert.match(prompt, /When the plan involves creating or editing source files, preserve the intended multiline structure and indentation instead of flattening code into one line\./u)
  assert.match(prompt, /Never compress or summarize large diffs, file reads, or other exact workspace state that the next step depends on\./u)
  assert.match(prompt, /Only repetitive terminal polling and similarly low-value command noise may be compacted\./u)
