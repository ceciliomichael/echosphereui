...
function resolveUnixShellSpecs() {
  ...
  const shells = [fromEnvironment, '/bin/zsh', '/bin/bash', '/bin/sh'].filter((value): value is string => {
...
function appendSessionOutputBuffer(activeSession: ActiveTerminalSession, chunk: string) {
  activeSession.outputBuffer += chunk
  if (activeSession.outputBuffer.length <= MAX_SESSION_OUTPUT_BUFFER_LENGTH) {
    return
  }

  const startIndex = activeSession.outputBuffer.length - MAX_SESSION_OUTPUT_BUFFER_LENGTH
  activeSession.outputBuffer = activeSession.outputBuffer.slice(startIndex)
}

function registerSessionWithOwner(ownerWebContentsId: number, sessionId: number) {
...
function unregisterSessionFromOwner(ownerWebContentsId: number, sessionId: number) {
  const activeOwnerSessions = ownerSessionIds.get(ownerWebContentsId)
  if (!activeOwnerSessions) {
    return
  }
...
function registerWorkspaceSession(ownerWebContentsId: number, workspaceKey: string, sessionId: number) {
  ...
  ownerWorkspaceSessions.set(ownerWebContentsId, new Map([[workspaceKey, sessionId]]))
}

function unregisterWorkspaceSession(ownerWebContentsId: number, workspaceKey: string, sessionId: number) {
  const ownerMappings = ownerWorkspaceSessions.get(ownerWebContentsId)
  if (!ownerMappings) {
    return
  }
...
function findWorkspaceSessionId(ownerWebContentsId: number, workspaceKey: string) {
  const ownerMappings = ownerWorkspaceSessions.get(ownerWebContentsId)
...
function terminateSession(sessionId: number) {
  ...
  unregisterWorkspaceSession(activeSession.ownerWebContentsId, activeSession.cwdKey, sessionId)
  try {
...
function assertSessionOwnership(ownerWebContentsId: number, sessionId: number) {
  ...
  if (activeSession.ownerWebContentsId !== ownerWebContentsId) {
    throw new Error(`Terminal session ${sessionId} does not belong to this window.`)
  }

  return activeSession
}

function spawnTerminalFromCandidates(input: { cols: number; cwd: string; env: NodeJS.ProcessEnv; rows: number }) {
...
function reuseExistingSession(input: {
  ...
  const existingSessionId = findWorkspaceSessionId(input.ownerWebContentsId, input.workspaceKey)
  if (existingSessionId === null) {
    return null
  }

  const activeSession = sessions.get(existingSessionId)
  if (!activeSession || activeSession.ownerWebContentsId !== input.ownerWebContentsId) {
    unregisterWorkspaceSession(input.ownerWebContentsId, input.workspaceKey, existingSessionId)
    return null
  }

  if (activeSession.ptyProcess.cols !== input.cols || activeSession.ptyProcess.rows !== input.rows) {
...
export async function createTerminalSession(
  ...
  const cols = clampInteger(input.cols, TERMINAL_MIN_COLS, TERMINAL_MAX_COLS, 120)
  const rows = clampInteger(input.rows, TERMINAL_MIN_ROWS, TERMINAL_MAX_ROWS, 30)
  ...
  registerWorkspaceSession(event.sender.id, workspaceKey, sessionId)

  ptyProcess.onData((data) => {
    const sessionForData = sessions.get(sessionId)
    if (!sessionForData || sessionForData.ownerWebContentsId !== event.sender.id || event.sender.isDestroyed()) {
      return
    }
  ...
  ptyProcess.onExit((exitEvent) => {
    ...
    unregisterWorkspaceSession(sessionForExit.ownerWebContentsId, sessionForExit.cwdKey, sessionId)
...
export async function writeToTerminalSession(event: IpcMainInvokeEvent, input: WriteTerminalSessionInput) {
  const activeSession = assertSessionOwnership(event.sender.id, input.sessionId)
  activeSession.ptyProcess.write(input.data)
}

export async function resizeTerminalSession(event: IpcMainInvokeEvent, input: ResizeTerminalSessionInput) {
  const activeSession = assertSessionOwnership(event.sender.id, input.sessionId)
  const cols = clampInteger(input.cols, TERMINAL_MIN_COLS, TERMINAL_MAX_COLS, activeSession.ptyProcess.cols)
  const rows = clampInteger(input.rows, TERMINAL_MIN_ROWS, TERMINAL_MAX_ROWS, activeSession.ptyProcess.rows)
  if (activeSession.ptyProcess.cols === cols && activeSession.ptyProcess.rows === rows) {
    return
  }
  activeSession.ptyProcess.resize(cols, rows)
}

export async function closeTerminalSession(event: IpcMainInvokeEvent, input: CloseTerminalSessionInput) {
  assertSessionOwnership(event.sender.id, input.sessionId)
  terminateSession(input.sessionId)
}

export async function openExternalTerminalLink(input: OpenExternalTerminalLinkInput) {
