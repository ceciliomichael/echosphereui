...
test('agent prompt encourages selective exploration instead of repeated root listings', () => {
  ...
  assert.match(prompt, /Treat every tool result as authoritative/u)
  assert.match(prompt, /successful write or edit result, the mutation already happened/u)
  assert.match(prompt, /Do not repeat a successful write or edit for the same path/u)
  assert.match(prompt, /Do not issue multiple write or edit calls for the same path in one response/u)
  assert.match(prompt, /After a successful tool call, choose the next step from that result/u)
})
