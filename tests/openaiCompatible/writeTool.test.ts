...
async function withTemporaryDirectory<T>(
  ...
  const directoryPath = await fs.mkdtemp(
    path.join(os.tmpdir(), "echosphere-write-tool-test-"),
  );
...
test("write tool formats multiline source files before writing them", async () => {
  await withTemporaryDirectory(async (workspacePath) => {
    ...
    assert.match(
      writtenContent,
      /<p>Build faster with the workspace formatter\.<\/p>/u,
    );
...
test("write tool normalizes CRLF content to LF for new files", async () => {
  await withTemporaryDirectory(async (workspacePath) => {
    const targetFilePath = path.join(workspacePath, "src", "crlf-new-file.ts");
...
test("write tool preserves existing CRLF line endings when overwriting", async () => {
  await withTemporaryDirectory(async (workspacePath) => {
    const targetFilePath = path.join(workspacePath, "src", "existing-crlf.txt");
    await fs.mkdir(path.dirname(targetFilePath), { recursive: true });
    await fs.writeFile(targetFilePath, "line one\r\nline two\r\n", "utf8");
