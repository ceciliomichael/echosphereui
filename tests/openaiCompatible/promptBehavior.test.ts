...
test('agent prompt encourages selective exploration instead of repeated root listings', () => {
  ...
  assert.match(prompt, /Do not repeat successful inspection calls .* with the same arguments unless workspace state changed/u)
  assert.match(prompt, /duplicate inspection call is blocked/u)
