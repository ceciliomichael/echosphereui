...
export async function formatWorkspaceFileContent(filePath: string, content: string, fallbackLineEnding?: LineEnding) {
  ...
  try {
    ...
    return convertToLineEnding(normalizeToLineFeed(formattedContent), sourceLineEnding)
  } catch {
    return content
  }
}
